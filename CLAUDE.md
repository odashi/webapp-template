...
- **IAP → Cloud Run invoker grant**: when `enable_iap = true`, the `lb` module grants the IAP service agent (`service-PROJECT_NUMBER@gcp-sa-iap.iam.gserviceaccount.com`) `roles/run.invoker` on both services (`google_cloud_run_v2_service_iam_member` in `lb/iap.tf`). The agent itself is created out of band by `/install` via `gcloud beta services identity create --service=iap.googleapis.com`. Because the Cloud Run services are created by Cloud Build (not Terraform), the binding is gated behind `grant_iap_invoker` (default `true`): `/install` passes `-var="grant_iap_invoker=false"` on the applies that precede the first deployment, then re-applies with the default once the services exist. Steady-state Cloud Build applies keep the binding because the default is `true`.
